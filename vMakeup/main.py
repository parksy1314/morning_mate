...
batch_size = 1
img_size = 256
no_makeup = cv2.resize(np.array(imageio.imread(args.no_makeup),dtype=np.uint8), (img_size, img_size))
...
for i in range(len(makeups)):
    makeup = cv2.resize(np.array(imageio.imread(makeups[i]),dtype=np.uint8), (img_size, img_size))
    Y_img = np.expand_dims(preprocess(makeup), 0)
    Xs_ = sess.run(Xs, feed_dict={X: X_img, Y: Y_img})
    Xs_ = deprocess(Xs_)
    result[:img_size, (i + 1) * img_size: (i + 2) * img_size] = makeup / 255.
    #result[:img_size, img_size: img_size] =Xs_[0] 
